...
# Strip balanced <think>...</think> blocks, and also drop an unterminated
# <think> (which happens when the reasoning is cut off by the token budget).
...
def _strip_reasoning(text: str) -> str:
    ...
    text = _THINK_OPEN_RE.sub("", text)  # unterminated block (truncated output)
    return text.strip()
...
def _complete(system: str, user: str, max_tokens: int = 800) -> str:
    ...
    if not cleaned:
        # The model spent its whole budget reasoning and never emitted a final
        # answer. Signal failure so the caller falls back cleanly instead of
        # returning a blank reply.
        raise RuntimeError("model returned only reasoning, no answer (raise max_tokens or use a non-reasoning model)")
    return cleaned
