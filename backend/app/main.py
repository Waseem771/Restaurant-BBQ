...
# Dashboard / analytics
# --------------------------------------------------------------------------
_PERIOD = Query("all", pattern=r"^(all|today|week|month|quarter)?$")


@app.get(f"{API}/dashboard/kpis", tags=["dashboard"])
def get_kpis(period: str = _PERIOD):
    return _run(analytics.kpis, period)


@app.get(f"{API}/sales/monthly", tags=["sales"])
def get_monthly(period: str = _PERIOD):
    return _run(analytics.revenue_by_month, period)
