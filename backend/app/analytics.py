...
def _period_bounds(period: str) -> tuple[str | None, str | None]:
    """Resolve a preset period name to inclusive ``(start, end)`` dates.

    Ranges are anchored to the dataset's latest ``order_date`` (not the wall
    clock) so a fixed sample dataset always populates. Returns ``(None, None)``
    for ``all`` / unknown, meaning "no date filter".
    """
    if not period or period == "all":
        return None, None
    row = db.query_rows("SELECT MAX(order_date) AS anchor FROM orders")[0]
    ...
    if period == "week":
        start = date.fromordinal(anchor.toordinal() - 6)
        return start.isoformat(), anchor.isoformat()
    if period == "month":
        return anchor.replace(day=1).isoformat(), anchor.isoformat()
    if period == "quarter":
        q_first_month = 3 * ((anchor.month - 1) // 3) + 1
        return anchor.replace(month=q_first_month, day=1).isoformat(), anchor.isoformat()
    return None, None


def kpis(period: str = "all") -> dict:
    """Headline KPIs for the dashboard, optionally scoped to a preset period."""
    start, end = _period_bounds(period)
    # Build a shared, parameterized date filter. Bounds come from
    # _period_bounds (derived from MAX(order_date)), so they are trusted, but
    # we still bind them rather than interpolate.
    where = ""
    params: tuple = ()
    if start and end:
    ...
    # order_items has no date column, so join orders to apply the same filter.
    profit_where = "WHERE o.order_date BETWEEN ? AND ?" if (start and end) else ""
...
def revenue_by_month(period: str = "all") -> list[dict]:
    start, end = _period_bounds(period)
    where = ""
    params: tuple = ()
    if start and end:
